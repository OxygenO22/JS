...
/* const solution = (firstArray, secondArray) => {
	...
	for (let i = 0; i < firstArray.length; i++) {
		res.push(Math.pow((firstArray[i] - secondArray[i]), 2));
	}

	return res.reduce((acc, cur) => acc + cur, 0) / firstArray.length;
} */

/// or

/* const solution = (a, b) =>  a.reduce((s, n, i) => s + Math.pow(n - b[i], 2), 0) / a.length; */
